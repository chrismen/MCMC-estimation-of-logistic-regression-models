...
// some temp variables
    ...
    b_time=-(a_time+1);
    sigma2=0.05;
    para_output = fopen ( "C-MCMC-time-seris-normal-version-1.txt", "w");
//-------------------------------------------------------------
    ...
    for ( g=1;g<=N; g++)
    {
        ...
        for ( k=0;k<(cols+1); k++)
        {
            ...
            for (h=0; h<rows; h++)
            {
                temp_new=beta_new[0] +beta_new[1]*x[h][0] +beta_new[2]*x[h][1]+beta_new[3]*x[h][2];
                temp_new=exp(temp_new);

                temp_current=beta_current[0] +beta_current[1]*x[h][0] +beta_current[2]*x[h][1]+beta_current[3]*x[h][2];
        ...
        if ( g>50)
        {
            ...
            fprintf (para_output, "%d  %f %f  %f %f %f \n",g, temp_new, beta_current[0],  beta_current[1], beta_current[2], beta_current[3]);
        }
